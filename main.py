...
async def ask_for_payment(target: Union[Message, CallbackQuery], state: FSMContext):
    ...
    try: await bot.delete_message(user_id, msg.message_id)
    except: pass
    ...
    if await check_subscription(user_id):
        ...
        try:
            async for msg in bot.get_chat_history(SOURCE_CHANNEL, limit=30):
                ...
                if text.startswith("📅Turnir sanasi"):
                    try: await bot.forward_message(message.chat.id, msg.chat.id, msg.message_id)
                    except: await message.answer(text)
                    break
        except Exception as e: logger.warning("Could not fetch channel history: %s", e)
        return
    await message.answer(
        "👋 Assalomu alaykum!\n\n"
...
# ----------------------------
# CALLBACKS: results / my_games / contact_admin
# ----------------------------
...
async def main_menu_return(message: Message):
    await message.answer("🔙 Asosiy menyu", reply_markup=inline_main_buttons)
...
def keep_alive():
    ...
    except Exception: logger.info("Keep-alive not started (environment may not allow)")
